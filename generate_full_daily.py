...
IMPACT_CHAIN_TEMPLATES = [
    ...
    {'triggers': ['ai', '算力', 'gpu', '大模型', '英伟达', 'nvidia', '人工智能', '机器学习', '深度学习', 'openai', 'gpt', 'deepseek', 'claude', 'gemini', 'llm', 'hbm', '智谱', '文心', '通义', 'kimi', '豆包', 'minimax'],
     'layers': [
         ...
         ('中小AI公司算力成本占营收超50%→倒闭潮→倒闭公司的二手GPU挂在闲鱼但买家不敢买(怕矿卡)，同时新入局者仍在加价3倍从黄牛手里抢新卡', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['芯片', '半导体', '台积电', '晶圆', '代工', '封装', '封测'],
     'layers': [
         ...
         ('国产替代需求爆发但良率不足→替代方案供不上→IC设计公司积压的旧型号芯片变成紧俏库存，终端厂被迫用国产但良率只有70%导致交货不良率高，旧型号芯片库存被抢光而新国产芯片月产能仅5万片', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['出口', '关税', '制裁', '贸易', '禁运', '脱钩', '实体清单', '反倾销', '反补贴', '中美', '外贸'],
     'layers': [
         ...
         ('原受限方卖不出去→库存积压+资金链紧→折价清仓但越南买家不知道谁在卖，同时越南替代供应商良率不够还没通过客户验证→中国卖方有货卖不出，美国买方有钱买不到', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['降息', '央行', '利率', '美联储', '加息', '流动性'],
     'layers': [
         ...
         ('宽松资金流向大企业→中小企业信贷仍缺钱→城商行有5%大额存单但起投门槛300万把散户挡在门外，中小企业缺周转资金但银行要求房产抵押→有钱人的钱趴在账上2%收益，缺钱的企业借民间借贷8%', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['ipo', '上市', '过会', 'a股', '港股', '招股', '融资', '天使轮', 'a轮', 'b轮', 'c轮', '并购', '收购', '入股', '投资'],
     'layers': [
         ...
         ('上市前老股东套现→老股转让估值差距大(同公司不同轮估值差30%)→买方不知道真实价格', '1-3月', '短缺'),
         ('解禁期→老股东需IR顾问→持牌IR顾问不到200人排期6个月→市值管理缺人', '1-3月', '短缺'),
         ('IPO数量增加→承销产能过剩→价格战→FA佣金从3%降到1%', '3-6月', '过剩'),
         ('解禁抛售→流动性不足→承接资金缺口→股价腰斩→急着卖的股东找不到足够买方，持牌IR顾问不到200人而解禁公司超2000家→排期到6个月后，急着减持的股东只能找地下私募接盘', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['庙宇', '供奉', '开光', '法会', '线上信仰', '财神', '赵公明', '信仰'],
     'layers': [
         ...
         ('庙宇IP商业化→授权体系空白→无标准无定价→寺庙方不知道IP能卖多少钱，文创公司不知道找寺庙哪个部门谈→全国没有一家IP授权中介懂宗教IP定价', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['台湾', '两岸', '小三通', '金门', '台海', '台积电', '台企', '陆资', '台商', '新竹', '台北'],
     'layers': [
         ...
         ('渠道重组→代购/集运需求爆发→供给不足→运费翻倍→淘宝商品到了厦门集运仓但找不到合规报关行拆箱清关→积压2周+，台商ODI额度每年100亿但3月就被抢光', '1-3月', '短缺'),
     ],
     'fracture': '物流断裂→兑汇渠道之间', 'window': '3-6个月'},

    # 14. 涨价/缺货/大宗（通用金属/化工，不含农产品——农产品走#17专用模板）
    # 因果链: 原料涨→加工减产→终端库存积压→替代品跟不上→普通人可做的回收/替代/清仓
    {'triggers': ['涨价', '缺货', '断供', '减产', '铜', '铝', '钢', '硫酸', '锂', '石油', '天然气', '铁矿石', '煤炭'],
     'layers': [
         ('供应减少→原料价格飙升→期货涨停→现货溢价（幅度看新闻原文，禁止编造）', '立即', '短缺'),
         ('原料涨→加工品成本上升→加工厂减产→中间品供给收紧', '1-2月', '短缺'),
         ('成本转嫁→终端需求下降→库存积压→经销商被迫折价清仓', '2-3月', '过剩'),
         ('替代品需求爆发→产能跟不上→爬坡期看品种实际产能周期（禁止统一写6个月）', '2-4月', '短缺'),
         ('回收需求上升→回收产能不足→废料闲置而冶炼厂缺原料（仅适用于金属品种，非金属品种禁止套用此条）', '2-4月', '短缺'),
     ],
    ...
    {'triggers': ['融资', '天使轮', 'a轮', 'b轮', 'c轮', '并购', '收购', '入股', '投资', '估值', '独角兽'],
     'layers': [
         ...
         ('IPO/并购退出→中介机构产能不足→排队6个月+→急着退出的老股东排不上队，有钱的基金找不到经过验证的标的→有钱的基金满手现金但尽调团队只有3人看不过来，优质项目找不到FA帮忙写BP→基金和项目卡在尽调产能上', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['政策', '新规', '监管', '合规', '牌照', '准入', '反垄断', '数据安全', '隐私'],
     'layers': [
         ...
         ('企业需要影响政策→游说/政策咨询缺口→没人做也没人教→企业不知道怎么应对新规，全国数据安全合规顾问不到500人但需整改企业超1.5万家→排队4个月+，企业只能花10万找律所出模板凑数', '3-6月', '短缺'),
     ],
     'fracture': '牌照稀缺→替代技术之间', 'window': '3-6个月'},

    # 17. 农产品/粮食 — 按品种拆分真实下游产业链，禁止万能模板
    # 17a. 大豆专用: 豆粕/饲料/养殖 + 豆油/食用油/生物柴油 + 大豆蛋白/食品加工
    {'triggers': ['大豆', '豆粕', '豆油'],
     'layers': [
         ('大豆减产/涨价→豆粕供给收紧（豆粕是大豆榨油副产品，占大豆价值70%）→饲料蛋白原料成本上升', '立即', '短缺'),
         ('豆粕涨价→饲料成本上升→养猪/养鸡利润压缩→禽畜减产→终端肉价上涨', '1-3月', '短缺'),
         ('大豆涨价→豆油供给收紧→食用油成本上升→食品加工企业（油炸/烘焙/方便面）成本飙升', '1-2月', '短缺'),
         ('豆油涨价→生物柴油原料成本上升（豆油是生物柴油主要原料之一）→生物柴油产量受限→混掺比例下降', '2-4月', '短缺'),
         ('大豆蛋白（分离蛋白/浓缩蛋白）涨价→植物肉/蛋白棒/运动营养品成本上升→替代动物蛋白需求但植物蛋白产能不足', '3-6月', '短缺'),
     ],
     'fracture': '豆粕供给收紧→饲料/养殖成本传导之间', 'window': '1-3个月'},

    # 17b. 糖专用: 糖醇比/乙醇 + 甜味剂替代 + 食品饮料
    {'triggers': ['白糖', '糖价', '蔗糖', '甜菜糖'],
     'layers': [
         ('糖减产/涨价→糖醇比变化（糖价>乙醇价时甘蔗优先产糖，反之转产乙醇）→巴西甘蔗分流乙醇→原糖出口减少', '立即', '短缺'),
         ('原糖出口减少→国际糖价进一步上涨→进口糖成本上升（中国/印尼等进口国受冲击最大）', '1-2月', '短缺'),
         ('糖价涨→食品饮料企业（糖果/烘焙/含糖饮料）原料成本上升→终端食品涨价或减糖配方替代', '1-3月', '短缺'),
         ('糖价涨→替代甜味剂需求上升（果葡糖浆/赤藓糖醇/三氯蔗糖）→但替代品产能爬坡需3-6个月→供给跟不上', '2-4月', '短缺'),
         ('糖价持续高位→下游用糖大户（奶茶/饮料连锁）利润压缩→转向自建甜味剂供应链或锁价远期合约', '3-6月', '短缺'),
     ],
     'fracture': '糖醇比变化→替代甜味剂供给之间', 'window': '2-4个月'},

    # 17c. 玉米/小麦/粮食通用: 饲料 + 食品 + 化工（淀粉/酒精）
    {'triggers': ['玉米', '小麦', '棉花', '稻谷', '油菜籽', '棕榈油',
                  '橡胶', '咖啡', '可可', '粮食', '化肥', '饲料', '收成', '收割', '播种',
                  '歉收', '丰收', '种植面积', '产区', '主产区'],
     'layers': [
         ('天气/产区变化→农产品减产或歉收→现货价格波动（幅度看新闻原文，禁止编造15%等数字）', '立即', '短缺'),
         ('农产品涨价→饲料成本上升→养殖业利润压缩→猪/鸡减产（仅适用于玉米/小麦等饲料原料，糖/棉花不适用）', '1-3月', '短缺'),
         ('粮价上涨→食品加工企业成本飙升→终端食品涨价（幅度看新闻原文）', '2-4月', '短缺'),
         ('化肥联动涨价→种植成本上升→下一季种植面积可能缩减', '3-6月', '短缺'),
         ('品种专用下游（玉米→淀粉/酒精/燃料乙醇；棉花→纺织/服装；咖啡/可可→饮品连锁）成本传导→终端涨价或替代', '3-6月', '短缺'),
     ],
     'fracture': '农产品减产→品种专用下游成本传导之间', 'window': '2-4个月'},
    ...
    {'triggers': ['突破', '首创', '世界首例', '里程碑', '颠覆', '革命性', '量产突破', '商用化',
     ...
     'layers': [
         ...
         ('旧技术路线被替代→相关产能变成沉没成本→设备/产线闲置→旧技术设备(如燃油发动机产线)挂在拍卖网半年无人问津，新技术专利诉讼排队12个月→3家巨头抢标准制定权但专利律师全国不到100人', '3-6月', '短缺'),
     ],
    ...
    {'triggers': ['海峡', '港口', '航线', '封锁', '通航', '禁运', '海上通道',
     ...
     'layers': [
         ...
         ('企业转向本地化库存策略→仓储需求暴增→但仓储容量不足→港口附近仓库月租金从30元/平涨到80元/平仍满租，有库存的卖方在等涨价不卖而买方在找货找不到→断裂在仓储容量和买卖双方不互通上', '1-3月', '短缺'),
     ],
    ...
    {'triggers': [],
     'layers': [
         ...
         ('转型/迁移需求→服务商排期2周+→有需求的企业在百度搜服务商但前3页都是广告公司，有库存的卖方挂在1688但买方在找现货→断裂在搜索可见度和现货交付能力上', '3-6月', '短缺'),
     ],
...
每条:
- **标题**
  > 📡 因果链: [因为今日新闻事件所以A→因为A所以B→因为B所以C→因为C所以D→因为D所以E，每步从新闻本身推导，禁止套用固定模板。同一条新闻里的具体数字/公司/政策名必须出现在因果链中。⚠️因果链禁止原地打转——新闻说X涨价/短缺，推导禁止停留在X本身（什么时候涨、涨多少），必须从X变化的后果往外推至少2级下游传导链，找到谁受伤、谁受益、哪里断裂。⚠️因果链必须沿着新闻品种的**真实下游产业链**推导——大豆推豆粕/豆油/大豆蛋白，糖推糖醇比/乙醇/甜味剂替代，铜推电缆/电机/电动车，禁止什么品种都推到"替代品爆发+回收产能不足"的万能句式。⚠️禁止编造新闻中没有的数字——如果新闻没说"涨15%"，推导里就不能出现15%]
...
- **因果链**: [因为今日新闻事件所以X短缺→因为X短缺所以Y短缺→因为Y短缺所以Z过剩...，每步从新闻具体内容推导，因果链中必须出现新闻里的具体公司/政策/数字。禁止套用"铜涨价→硫酸涨价→磷肥涨价"这类固定模板。⚠️因果链必须沿着新闻品种的真实下游产业链推导，禁止万能大宗模板。⚠️禁止编造新闻中没有的数字]
  - 断裂在: [哪两步之间供需最不平衡]
...
> 从今日最高分新闻出发，推导因果传导链。必须写清每步的因果关系（因为A所以B），禁止只列断言（A短缺、B短缺）。必须结合今日具体新闻，禁止抽象模板。因果链中必须出现新闻里的具体公司名/政策名/数字，如果因果链里没有今日新闻的具体内容，说明你在套模板。⚠️因果链禁止原地打转——新闻描述的现象是起点不是终点，必须从新闻现象的后果往外推至少2级下游传导，找到谁受伤、谁受益、哪里断裂。⚠️因果链必须沿着新闻品种的真实下游产业链推导——大豆→豆粕/豆油/大豆蛋白，糖→糖醇比/乙醇/甜味剂，铜→电缆/电机/电动车，禁止什么品种都推到"替代品爆发+回收产能不足"。⚠️禁止编造新闻中没有的数字。
    ...
    try:
        ...
        if content and len(content) > 500:
            ...
            if not missing:
                ...
                # V20-fix13: AI输出也做自嗨检测
                has_vague, vague_hits = _detect_vague_phrases(content)
                if has_vague:
                    unique_hits = list(set(vague_hits))
                    content += f"\n\n---\n⚠️ **自嗨检测**: 发现 {len(unique_hits)} 处空话套话: {', '.join(unique_hits)}\n> 邪修点评: 这些词放之四海而皆准，等于什么都没说。请检查对应位置的因果链是否基于今日新闻推演。\n---"
                    logging.warning(f"[日报] AI输出自嗨检测: 发现空话套话 {unique_hits}")
                # 记录邪修内容
...
# V20-fix13: 自嗨检测 — 检测输出中的空话套话
VAGUE_PHRASES = [
    '供需断裂', '信息不对称', '供需两端', '配套服务供给不足', '信息断裂',
...
def _detect_vague_phrases(text):
    """V20-fix13: 检测输出中的空话套话，返回(是否检测到, 命中词列表)"""
    hits = [vw for vw in VAGUE_PHRASES if vw in text]
    return (len(hits) > 0, hits)


def _inject_news_entity_into_chain(chain_text, news_title):
    """V20-fix13: 把新闻标题里的实体/数字注入因果链，避免完全脱离新闻

    策略: 从标题提取公司名/数字/政策名，在因果链开头加"因[今日新闻]→"
    让读者知道这条链是从哪条新闻推导出来的。
    """
    ...
    # 提取标题中的数字（价格/百分比/数量）
    import re
    numbers = re.findall(r'\d+[%万台币元万千百亿\./]+', news_title)
    # 提取标题中的公司名/政策名（简单规则：含"公司/集团/部/委/总局/办"的片段）
    entities = re.findall(r'[\u4e00-\u9fff]{2,8}(?:公司|集团|部门|总局|办公室|委员会|部委)', news_title)

    # 如果有数字或实体，在因果链前注入新闻标签
    inject_parts = []
    if entities:
        inject_parts.append(entities[0])
    if numbers:
        inject_parts.append(numbers[0])

    if inject_parts:
        inject_tag = '今日新闻(' + '/'.join(inject_parts[:2]) + ')→'
        # 如果因果链开头不是"今日新闻"，就注入
        if not chain_text.startswith('今日新闻'):
            chain_text = inject_tag + chain_text
...
def _fallback_all_sections(all_raw, top_items):
    ...
    # V20-fix13: 自嗨检测 — 输出前扫描空话套话
    full_output = "\n".join(sections)
    has_vague, vague_hits = _detect_vague_phrases(full_output)
    if has_vague:
        unique_hits = list(set(vague_hits))
        warning = f"\n\n---\n⚠️ **自嗨检测**: 发现 {len(unique_hits)} 处空话套话: {', '.join(unique_hits)}\n> 邪修点评: 这些词放之四海而皆准，等于什么都没说。AI模式应基于今日新闻替换为具体内容。\n---"
        full_output += warning
        logging.warning(f"[日报] 自嗨检测: 发现空话套话 {unique_hits}")
...
要求：
- 每行是一个独立结论，不超过 20 字
- 不出现"根据今日资讯"、"据分析"这类废话开头
- 直接输出 5 行，不要编号不要解释不要分隔线
- 结论要具体，要有信息量，不能是大白话
- 禁止把结论往台湾那里套，结论必须来自今日新闻具体内容，不能套万能模板
