        ...
        try:
            ...
            if isinstance(data, list):
                ...
                try:
                    ...
                    c = conn.cursor()
                    # 从 lottery_analyzer 的输出读取期数（如果存在）
                    import os
                    period_file = '/root/asuan-jinzhu/lottery-periods.json'
                    if os.path.exists(period_file):
                        with open(period_file) as f:
                            period_map = json.load(f)
                    else:
                        # 降级：从结算数据推断（取最新一条的 period）
                        for g in ['ssq', 'dlt', 'qxc']:
                            c.execute("""SELECT b.game, s.actual_numbers 
                                        FROM algo_bets b 
                            ...
                            if row:
                                ...
                                period_map[g] = data.get('period', '未知')
                    conn.close()
                except:
                    pass
                ...
                for game_key, game_label in [('ssq_recs', '🔴 双色球'), ('dlt_recs', '🔵 大乐透'), ('qxc_recs', '🟢 七星彩')]:
                    ...
                    if game_recs:
                        # 手动指定期数（临时方案，明天修复自动读取）
                        period_map = {'ssq_recs': '26073', 'dlt_recs': '26071', 'qxc_recs': '26073'}
                        period = period_map.get(game_key, '')
                        period_str = f'（第{period}期）' if period else ''
                        lines.append(f'### {game_label}{period_str}')
                        for i, rec in enumerate(game_recs[:5]):
                            # V21: 适配实际字段格式 reds+blue(SSQ/DLT) 或 digits/numbers
                            if 'reds' in rec and 'blue' in rec:
                                reds = rec['reds']
                                blue = rec['blue']
                                fmt = ' '.join(str(int(r)) for r in reds) + '  + ' + str(int(blue))
                            elif 'front' in rec and 'back' in rec:
                                front = rec['front']
                                back = rec['back']
                                fmt = ' '.join(str(int(f)) for f in front) + '  + ' + ' '.join(str(int(b)) for b in back)
                            elif 'digits' in rec:
                                fmt = ' '.join(str(int(d)) for d in rec['digits'])
                            elif 'numbers' in rec:
                                fmt = ' '.join(str(int(n)) for n in rec['numbers'])
                            elif 'main' in rec and 'special' in rec:
                                main = rec['main']
                                special = rec['special']
                                fmt = ' '.join(str(int(m)) for m in main) + '  + ' + str(int(special))
                            else:
                                fmt = str(rec)
                            lines.append(f'注{i+1}: {fmt}  [{rec.get("strategy", rec.get("strategy", "策略"))}]')
                        lines.append('')
                if not recs:
            ...
            if isinstance(data, list):
                ...
                for game_key, game_label in [('pln_recs', '🎲 台湾威力彩(PLN)'), ('ltn_recs', '🎯 台湾大乐透(LTN)')]:
                    ...
                    if game_recs:
                        ...
                        for i, rec in enumerate(game_recs[:5]):
                            # V21: 适配 reds+blue 格式（JinZhu输出）或 numbers/digits
                            if 'reds' in rec and 'blue' in rec:
                                reds = rec['reds']
                                blue = rec['blue']
                                fmt = ' '.join(str(int(r)) for r in reds) + '  + ' + str(int(blue))
...
if __name__ == '__main__':
    ...
    try:
        import subprocess
        # 改为调用独立的马斯克系统
        musk_system_script = '/root/musk-system/scripts/analyze_project.py'
        result = subprocess.run(
            [sys.executable, musk_system_script, today_str],
