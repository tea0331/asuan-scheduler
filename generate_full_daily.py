...
每条:
- **标题**
  > 📡 因果链: [因为今日新闻事件所以A→因为A所以B→因为B所以C→因为C所以D→因为D所以E，每步从新闻本身推导，禁止套用固定模板。同一条新闻里的具体数字/公司/政策名必须出现在因果链中。⚠️因果链禁止原地打转——新闻说X涨价/短缺，推导禁止停留在X本身（什么时候涨、涨多少），必须从X变化的后果往外推至少2级下游传导链，找到谁受伤、谁受益、哪里断裂]
...
> 从今日最高分新闻出发，推导因果传导链。必须写清每步的因果关系（因为A所以B），禁止只列断言（A短缺、B短缺）。必须结合今日具体新闻，禁止抽象模板。因果链中必须出现新闻里的具体公司名/政策名/数字，如果因果链里没有今日新闻的具体内容，说明你在套模板。⚠️因果链禁止原地打转——新闻描述的现象是起点不是终点，必须从新闻现象的后果往外推至少2级下游传导，找到谁受伤、谁受益、哪里断裂。
...
7. ❌ 融资PR/股票盘前涨跌/普通产品发布PR/会议预告/人事变动/喊话表态——禁止推送，无因果推演价值
8. 只推送10类高价值触发源: 重大签署/国内新规/AI算力信息化/供需断裂/产能变化/地缘通道/农产品贵金属产量预警/新发明新应用/行业数据/国内国外缺货
9. ⭐ 重大签署类新闻（国家间协议/贷款/合作）必须深度推演因果链
10. ⭐ 国内新政策/新规类新闻（国务院/部委/省级）必须深度推演因果链
11. ⭐ 农产品/贵金属产量预警（减产/歉收/种植面积/开采量变化）必须深度推演因果链
12. ⭐ 新发明/新应用/新产品(全球关注)必须深度推演因果链
13. 注意: 普通产品发布(手机更新/SaaS上线/小版本迭代)≠新发明新应用，前者禁止后者必须推演
14. 金句每天不同，结合当日主题
15. 总字数2000-3000字
16. ⏰ 所有时间窗口基于{today_str}推算
17. 📍 优先挖掘台湾相关机会
{scene_context}
{chain_ctx}"""
