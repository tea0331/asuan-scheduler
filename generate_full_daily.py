...
def score_news_with_scene(item):
    ...
    # 已知项目关联加权
    if any(kw in text for kw in ['加盟', '餐饮', '甜品',
                                   '绵绵冰', '冰淇淋', '连锁']):
        scene_bonus += 2
...
LIU_DOMAINS = [
    ...
    ('餐饮/加盟', ['餐饮', '甜品', '小吃', '冰淇淋', '奶茶', '冷链', '加盟', '代理', '连锁', '中央厨房', '预制菜', '餐饮出海'], 2),
...
CHAIN_TEMPLATES = [
    ...
    # 链10: 餐饮加盟→连锁扩张
    {'trigger': ['加盟', '连锁', '餐饮', '代理'],
     'chain': ['餐饮品牌开放加盟/区域代理', '加盟费+供应链利润模式成熟', '冷链/中央厨房配套需求', '线下体验+线上私域融合', '品牌溢价→消费升级'],
...
- 当前项目: 机票/酒店/电话卡(已办)/台湾彩种分析/金车威士忌调配/赵公明线上庙宇
...
def _infer_signal(title):
    ...
    if any(kw in title_lower for kw in ['机器人', '具身智能', '智能体']):
        ...
        return templates[seed % len(templates)]
    elif any(kw in title_lower for kw in ['加盟', '代理', '连锁']):
        templates = [
            f'区域代理: 以「台湾区域代理商」联系{ent}→谈独家代理→首付<10万→赚信息差 | 第一步: 查{ent}是否已有台湾加盟店(经济部商业司) | 预期首付5-10万+供应链差价10-15%',
            f'冷链对接: {ent}扩张需冷链→台湾冷链有闲置→以「两岸冷链对接」撮合→收3% | 第一步: 联系台湾冷链协会(www.cla.org.tw)问会员名单 | 预期介绍费3%+运费抽成',
            f'市场调研: 品牌方缺台湾市场数据→以「台湾餐饮市场调研」卖报告→5-8万/份 | 第一步: 查台湾财政部营利事业家数(餐饮业) | 预期5-8万/份',
        ]
        return templates[seed % len(templates)]
...
def _fallback_gap_scan(top_items):
    ...
    for kw, context in signal_keywords.items():
        ...
        if kw in ['台湾', '两岸', '小三通']:
            ...
            lines.append(f"- **缺口**: {ent_short}→区域代理 — {context[:30]}")
...
def _fallback_deep_chain(top_items):
    ...
    if '台湾' in source or any(kw in title_lower for kw in ['台湾', '两岸', '台']):
        ...
        layer2 = f"第2层（直接影响）: {ent}→两岸人流/物流/资金流短期调整→小三通货运量波动→台币/人民币民间汇兑需求激增"
        layer3 = f"第3层（产业链传导）: 台商资金重新配置→{aux_ent}产能调整→金门/厦门仓储转运需求上升→替代供应商崛起"
        layer4 = f"第4层（跨产业传导）: 两岸服务业(旅游/餐饮/金融)→汇率避险→人民币/台币跨境结算→{aux_ent2}业务量扩大"
        layer5 = f"第5层（终局推演）: 两岸官方渠道越不通→{ent}相关的民间灰色通道越赚钱→小额高频交易取代大宗贸易→谁掌握通道谁收税"
        ...
        layer1 = f"第1层（事件）: {title}"
        layer2 = f"第2层（直接影响）: {ent}价格波动→下游{aux_ent}成本上升→终端产品定价调整→部分中小企业停产/减产"
        layer3 = f"第3层（产业链传导）: 替代材料需求上升→{aux_ent2}利润暴增→物流重新配置→库存投机商入场囤货"
        layer4 = f"第4层（跨产业传导）: {ent}成本上升→物流/包装涨价→{aux_ent}提价→消费者转向替代品→二手/翻新市场活跃"
        layer5 = f"第5层（终局推演）: {ent}供需断裂处就是中间人的收钱位置→第一个找到替代方案的人赚最多→囤货的赚价差、撮合的赚佣金"
        ...
        xie_xiu = f"邪修之道: 刘老板在台湾→利用地理信息差→①锁定{ent}②以「台湾连锁餐饮集团采购总监」身份谈独家代理③首付压到10万以内④冷链找共享仓不自己建⑤品牌方要直营时把代理权卖回给他们→赚代理权转让费+供应链差价"
        ...
        layer1 = f"第1层（事件）: {title}"
        layer2 = f"第2层（直接影响）: {ent}拉动算力需求→GPU供不应求→大陆受出口管制→台湾/新加坡IDC需求暴增"
        layer3 = f"第3层（产业链传导）: 散热/电源/PCB→{aux_ent}数据中心扩张→电力→核能/绿电→二手GPU灰色市场→算力租赁中介"
        layer4 = f"第4层（跨产业传导）: {ent}应用落地→{aux_ent2}效率提升→岗位替代→新工种→教育/培训→办公室/商业地产需求变化"
        layer5 = f"第5层（终局推演）: 卖铲子的({ent}算力/工具)稳赚→淘金的(AI应用)99%会死→算力灰色通道是{ent}信息差最大的环节"
...
def _fallback_quote(top_items):
    ...
    elif any(kw in title_lower for kw in ['加盟', '餐饮']):
        candidates = [
