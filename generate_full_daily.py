...
def _fetch_rss(url, count=5, timeout=8):
    """从RSS源获取新闻（用curl下载，再feedparser解析）"""
    try:
        import feedparser, tempfile, subprocess
        
        # 用curl下载
        with tempfile.NamedTemporaryFile(mode='wb', suffix='.xml', delete=False) as f:
            temp_file = f.name
        
        cmd = ['curl', '-s', '-L', '-A', 'Mozilla/5.0', '--max-time', str(timeout), '-o', temp_file, url]
        result = subprocess.run(cmd, timeout=timeout+2)
        
        if result.returncode != 0:
            logging.warning(f"[新闻] RSS下载失败({url}): curl错误")
            return []
...
def _fetch_baidu_hot(count=10):
    ...
    try:
        ...
        cmd = ['curl', '-s', '-L', '-A', headers, '--max-time', '10', '-o', temp_file, url]
        result = subprocess.run(cmd, timeout=12)
        
        if result.returncode != 0:
            logging.warning('[新闻] 百度热搜curl失败')
            return []
...
def generate_news_section():
    """基于RSS+热搜抓取原始素材，用AI生成前两板块（每日资讯+市场缺口）"""
    ...
    # 构建素材文本（喂给AI）
    material_lines = []
    for i, n in enumerate(unique[:15], 1):  # 减少素材，避免prompt太长
        score = score_news(n)
        title = n['title'].strip()
    ...
    # ---- AI生成两大板块 ----
    system_msg = """你是刘海蟾点金的邪修分析师——专门做中间人流水生意的操盘手。

你的生意模式：不碰货、不碰生产、只站在资金流和信息流中间收过路费。

铁律：
- 落地动作是操盘级的：注册什么主体/签什么协议/怎么走账
- 禁用废话：值得关注/需警惕/赋能/生态"""
...
每条：
- **标题**（搞钱角度）
  > 💰 落地动作：资金流怎么走（谁付钱/付多少/怎么收/合同名目）
  > 🕳️ 缺口：资金断裂/资质壁垒/信息不对称
...
- ⚖️ 风险与规避：灰度🟢🟡🔴 + 法律红线"""
    ...
    logging.info("[新闻] 开始调用混元API（只生成前两板块）...")
    content = _call_hunyuan_api(system_msg, user_msg, timeout=90)
