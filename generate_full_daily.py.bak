                    ...
                    if game_recs:
                        ...
                        for i, rec in enumerate(game_recs[:5]):
                            ...
                            if 'reds' in rec and 'blue' in rec:
                                reds = rec['reds']
                                blue = rec['blue']
                                fmt = ' '.join(str(int(r)) for r in reds) + '  + 特别号 ' + str(int(blue))
                            elif 'numbers' in rec:
                                nums = rec['numbers']
                                if isinstance(nums, list) and len(nums) >= 7:
                                    main_nums = nums[:6]
                                    special = nums[6]
                                    fmt = f"主号 {' '.join(str(int(n)) for n in main_nums)} + 特别号 {special}"
                                else:
                                    fmt = ' '.join(str(int(n)) for n in nums) if isinstance(nums, list) else str(rec)
                            elif 'digits' in rec:
                                fmt = ' '.join(str(int(d)) for d in rec['digits'])
                            else:
                                fmt = str(rec)
