...
# hy3-preview API (从 openclaw config 读取)
...
async def fetch_comments(bvid, aid, top_n=20):
    ...
    try:
        # 使用 comment 模块获取热门评论
        from bilibili_api.comment import get_comments, CommentResourceType, OrderVideo
        comments_data = await get_comments(
            type_=CommentResourceType.VIDEO,
            oid=aid,
            order=OrderVideo.LIKE  # 按点赞数排序
        )
